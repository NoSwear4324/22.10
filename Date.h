...
public:
	...
	//--------- Œœ≈–¿“Œ–» œ–»—¬Œ™ÕÕﬂ ---------	
	Date& operator += (int days) // ‰Ó‰‡ÚË days ‰Ì≥‚ ‰Ó ‰‡ÚË
	{
	...
	Date& operator -= (int days) // ‚≥‰ÌˇÚË days ‰Ì≥ ‚≥‰ ‰‡ÚË
	{
		for (int i = 0; i < days; i++)
		{
			this->prevDate();
	...
	Date& operator += (float months) // ‰Ó‰‡ÚË months Ï≥Òˇˆ≥‚ ‰Ó ‰‡ÚË
	{
		this->month += month;
		if (this->month > 12)
		{
	...
	Date& operator -= (float months) // ‚≥‰ÌˇÚË months Ï≥Òˇˆ≥ ‚≥‰ ‰‡ÚË
	{
		this->month -= month;
		if (this->month <= 0) {
	...
	Date& operator += (long years) // ‰Ó‰‡ÚË years ÓÍ≥‚ ‰Ó ‰‡ÚË
	{
		this->year += years;
		return *this;
	}
	Date& operator -= (long years) // ‚≥‰ÌˇÚË years ÓÍ≥‚ ‚≥‰ ‰‡ÚË
	{
		this->year -= years;
		return *this;
	}
